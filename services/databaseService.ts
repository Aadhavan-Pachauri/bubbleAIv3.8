...
export const getAllChatsForUser = async (supabase: SupabaseClient, userId: string): Promise<ChatWithProjectData[]> => {
    // UPDATED: Now also needs to consider chats where the user is a participant but not the owner.
    // However, for simplicity without complex SQL views, we are currently fetching owned chats.
    // If 'chat_participants' is implemented properly with RLS, we could select based on that join.
    // For now, staying safe with existing logic + visual tags.
...
export const addChatParticipant = async (supabase: SupabaseClient, chatId: string, userId: string): Promise<void> => {
    // 1. Mark chat as group
    const { error: updateError } = await supabase
        .from('chats')
        .update({ is_group: true, updated_at: new Date().toISOString() })
        .eq('id', chatId);
        
    if (updateError) handleSupabaseError(updateError, 'Failed to convert chat to group');

    // 2. Add participant (assumes chat_participants table exists)
    const { error: insertError } = await supabase
        .from('chat_participants')
        .insert({ chat_id: chatId, user_id: userId });

    if (insertError) {
        // If table doesn't exist, we fail gracefully for the UI but log it
        console.error("Could not add participant to relational table:", insertError);
        // We throw to let the UI know it failed partially or completely
        if (insertError.code === '42P01') { // undefined_table
             throw new Error("Group chat features require a database update. Please contact the administrator.");
        }
        handleSupabaseError(insertError, 'Failed to add participant');
    }
};
...
export const getFriendships = async (supabase: SupabaseClient, userId: string): Promise<Friendship[]> => {
    // Assuming 'friendships' table joins 'profiles' on friend_id
    const { data, error } = await supabase
        .from('friendships')
        .select('*, other_user:profiles!friend_id(*)') 
        .eq('user_id', userId)
        .eq('status', 'accepted');
...
export const getPendingFriendRequests = async (supabase: SupabaseClient, userId: string): Promise<any[]> => {
    // Incoming requests: friend_id is me, status is pending. Sender is user_id.
    const { data, error } = await supabase
        .from('friendships')
...
export const sendPrivateMessage = async (supabase: SupabaseClient, senderId: string, recipientId: string, content: string): Promise<PrivateMessage> => {
    const { data, error } = await supabase
        ...
        .insert({ 
            ...
            content,
            conversation_id: [senderId, recipientId].sort().join(':') // Simple convo ID generation
        })
        .select()
        .single();
